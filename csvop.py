...
def get_file_instance(mode="FRESH"):
    ...
    if value:
        try:
            return open('employee.csv', value)  # w-- if not present-- will create new file
        except:
...
class EmpCSVServiceImpl(EmpService):
    ...
    def add_employee(self, emp):
        if type(emp) == Employee:
            ...
            print('Emp <{}> added into CSV file..!'.format(emp.empId))
        else:
    ...
    def delete_employee(self, empid):  # ID
        ...
        if femp:  # PRESENT - ASEL TR
            ...
            print('Specificed employee id not present so cant remove')
        ...
        if femp:
            ...
            for emp in allemps:
                if emp.empId == empid:
                    ...
                    file = get_file_instance('FRESH')  # EXISTING DATA  # EXISTING DATA REMOVE N CREATE NEW FILE
                    for emp in allemps:  # WRITE ALL DATA EXCEPT-- DELETED EMP
                        empinfo = str(emp.empId) + SEPERATOR + emp.empName + SEPERATOR + str(
                            emp.empAge) + SEPERATOR + str(
                            emp.empSalary) + SEPERATOR + emp.empRole + "\n"
                        file.write(empinfo)
                    break
        else:
...
if __name__ == '__main__':
    csvservice = EmpCSVServiceImpl()
    for no in range(3):
        csvservice.add_employee(get_input_from_user(idwant=True))
